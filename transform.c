...
{
	/* Bring back matrix transformation
	 * could be useful for sideway panel devices, or just in general */
	float m[9];
	float v[3];
	float v_new[3];
	...
	switch (sensor_desc[s].type) {
		case SENSOR_TYPE_ACCELEROMETER:
			property_get("ro.vendor.iio.accel.matrix", cm, "1,0,0,0,1,0,0,0,1");
		...
		case SENSOR_TYPE_GYROSCOPE:
			property_get("ro.vendor.iio.anglvel.matrix", cm, "1,0,0,0,1,0,0,0,1");
			v[0] = data->gyro.x;
			v[1] = data->gyro.y;
			v[2] = data->gyro.z;
			break;
		...
		case SENSOR_TYPE_MAGNETIC_FIELD:
			property_get("ro.vendor.iio.magn.matrix", cm, "1,0,0,0,1,0,0,0,1");
			v[0] = data->magnetic.x;
			v[1] = data->magnetic.y;
			v[2] = data->magnetic.z;
			break;
	...
	if (sensor_desc[s].type == SENSOR_TYPE_ACCELEROMETER ||
		sensor_desc[s].type == SENSOR_TYPE_MAGNETIC_FIELD ||
		sensor_desc[s].type == SENSOR_TYPE_GYROSCOPE) {
		sscanf(cm, "%f,%f,%f,%f,%f,%f,%f,%f,%f",
	...
	/* Light sensor scale prop, if the driver provided scale (or lack thereof)
	 * do not match with the one size fit all config_autoBrightnessLevels */
	if (sensor_desc[s].type == SENSOR_TYPE_LIGHT) {
		property_get("ro.vendor.iio.light.scale", cm, "1.0");
